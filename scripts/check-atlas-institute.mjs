...
try {
  for (const width of [1440, 390]) {
    ...
    for (const name of names) {
      ...
      await checkbox.waitFor(); assert.equal(await checkbox.isChecked(), true, name);
    }
    ...
    console.log(`PASS ${width}px: six Institut defaults, source labels, section ordering, water pins, overlays and toggles`);
    await page.close();
  }
