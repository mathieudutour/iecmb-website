...
import { clipTrafficPaths, trafficColor, trafficValue, TRAFFIC_ROADS } from '../src/lib/road-traffic.ts';
import { parseRoadTraffic, loadRoadTraffic, roadTrafficUrl } from '../src/lib/road-traffic-source.ts';
...
test('traffic colours use consistent volume classes; missing counts are never zero or green', () => {
  ...
  assert.equal(TRAFFIC_ROADS.length,7);
});
...
test('GML import keeps annual counts, correct latitude/longitude order, seven exact roads and source IDs', () => {
  ...
  assert.deepEqual(data.segments[0].paths,[[[45.9,6.5],[45.95,6.6]]]);
...
test('unidentified geometries get stable distinct IDs and out-of-area lines are excluded', () => {
  const a=member({id:'X'}),b=member({id:'X',points:'46 6.6 46.05 6.7'});
