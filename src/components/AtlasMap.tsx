...
import { Layers, LocateFixed, ExternalLink, MapPin, Wind, Droplets, Shovel } from "lucide-react";
...
const LAYER_GROUPS = [
  ...
  { id: "air", title: "Air", icon: Wind, layers: ["atmo", "wood", "traffic"] },
...
export default function AtlasMap({ inventory, bathing, riverAssessments, riverCatalogue, traffic }: { inventory: PollutionSitesResult | null; bathing: BathingData; riverAssessments: RiverAssessments; riverCatalogue: RiverCatalogue; traffic: RoadTrafficData }) {
  ...
  const activeCount = Object.values(enabled).filter(Boolean).length + bioDemo.activeCount + soilDemo.activeCount;
  ...
  {selection && <AtlasDetailDialog
    ...
    {selection.kind === "atmo" ? <AtmoDetails state={atmo} stationId={selection.id} /> : selection.kind === "inventory" ? <AtlasInventoryDetails site={selection.site} /> : selection.kind === "traffic" ? <RoadTrafficDetails segment={selection.segment} data={traffic} /> : selection.kind === "bathing" ? <AtlasBathingDetails point={selection.point} /> : selection.kind === "drinking" ? <AtlasDrinkingDetails point={selection.point} summary={drinkingSummaries[selection.point.id]} /> : <AtlasWaterDetails
