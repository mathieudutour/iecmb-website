...
import { INSTITUTE_WATER_LAYERS, INSTITUTE_WATER_SITES, type InstituteWaterKind, type InstituteWaterSite } from "@/atlas/lib/institute-water-demo";
import InstituteReport from "./InstituteReport";
import { waterInstituteReport } from "@/atlas/lib/institute-report";

export function useInstituteWaterDemo() {
  const [enabled, setEnabled] = useState<Record<InstituteWaterKind, boolean>>({ bathing: false, rivers: false });
  const [selected, setSelected] = useState<InstituteWaterSite | null>(null);
  const controls = INSTITUTE_WATER_LAYERS.map((layer) => <EvidenceLayerControl key={layer.id} title={layer.title} source={INSTITUTE_DEMO_SOURCE} enabled={enabled[layer.id]} onEnabled={(value) => setEnabled((prev) => ({ ...prev, [layer.id]: value }))}>
